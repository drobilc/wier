...
def printDBData(dataFromDB,tokenizedQuery):
    #v bistvu bi lahko združil zapise istih datotek
    zdruzeni=[]
    for i in  dataFromDB:
    ...
    #tukaj moram še očititi tiste stringe in indekse, ki se prekrivajo
    #malce po zgledu izpisa iz baze
    for i in zdruzeni:
        i.append([])
        i.append([])
        i[6]=[len(i[1])]
    ...
    #sortiram po številu besed
    zdruzeni.sort(key=lambda row: (row[6]), reverse=True)

    #moral bi še sortirati znotraj tako, da bi se snippeti, ki imajo več besed, pojavili preje.
    #se pravi vrstico 3 in 4
    for i in zdruzeni:
    ...
    for i in zdruzeni:
        ...
        #print(str(i[4]).ljust(11), " + ".join(i[1]).ljust(len("   ".join(tokenizedQuery))+5), (i[0][0]).ljust(50), " ... ".join(i[3]).ljust(100))
